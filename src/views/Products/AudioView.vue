<template>
  <div class="audio-page">
    ...
    <div v-if="hasActiveFilters" class="active-filters">
      <div class="container">
        ...
        <div class="active-filters-list">
          <div v-for="(value, key) in filters" v-if="value && key !== 'free_shipping'" :key="key" class="filter-tag">
            <span>{{ value }}</span>
    ...
    <div class="content-wrapper">
      <FiltersSidebar :filterGroups="filterGroups" v-model="filters" @update="applyFilters" />
      ...
      <section class="products-section">
        ...
        <ProductGrid :products="products" :formatPrice="formatPrice" @select="goToProduct" @img-error="onImgError" />
...
export default defineComponent({
  ...
  setup() {
    ...
    const validCategories = ['Auriculares', 'Altavoces', 'Soundbars', 'Accesorios de Audio'];

    const categoriesMap: Record<string, string> = {
      'Auriculares': 'headphones',
      'Altavoces': 'speakers',
      'Soundbars': 'soundbars',
      'Accesorios de Audio': 'audio_accessories'
    };
    ...
    const filterGroups = [
      ...
      { title: 'Precio', key: 'price', options: ['Menos de $200.000', '$200.000 - $500.000', '$500.000 - $1.000.000', 'Más de $1.000.000'] },
      { title: 'Marca', key: 'brand', options: ['Sony', 'Bose', 'JBL', 'Sennheiser', 'Beats', 'Samsung'] },
    ...
    const breadcrumbs = computed(() => {
      ...
      if (filters.value.category && validCategories.includes(filters.value.category)) {
        crumbs.push({ text: filters.value.category, link: `/audio?category=${encodeURIComponent(filters.value.category)}` });
      }
      return crumbs;
    });
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (filters.value.free_shipping) params.free_shipping = true;
    ...
    const clearFilter = (key: string) => { filters.value[key as keyof typeof filters.value] = key === 'free_shipping' ? false : ''; applyFilters(); };
    const clearAllFilters = () => { Object.keys(filters.value).forEach(k => filters.value[k as keyof typeof filters.value] = k === 'free_shipping' ? false : ''); applyFilters(); };
